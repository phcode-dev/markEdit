...
window.crepe = new Crepe({
    root: '#app',
    defaultValue: markdown
});
crepe.editor.use(listener);

window.list = listener;


// Function to calculate changed lines
function getChangedLines(previous, current) {
    ...
    for (let i = 0; i < maxLength; i++) {
        const prevLine = previousLines[i] || ""; // Default to empty if line doesn't exist
...
function interactWithEditor(editor) {
    const ctx = editor.ctx;

    const listener = ctx.get(listenerCtx);

    // Listen for changes in Markdown
    listener.markdownUpdated((ctx, markdown, previousMarkdown) => {
        console.log("Markdown changed!");
        // console.log("Previous:", previousMarkdown);
        // console.log("Current:", markdown);
        // Get changed lines
        const changes = getChangedLines(previousMarkdown, markdown);
        console.log("Changed Lines:", changes, JSON.stringify(changes));
    });

    // Get current Markdown
    const getMarkdown = utils.getMarkdown();
    const currentMarkdown = getMarkdown(ctx);
    console.log("Markdown content:", currentMarkdown);

    // Replace part of the Markdown
    const updatedMarkdown = currentMarkdown.replace(
        '## Javascript Code Intelligence',
        '## Typescript Code Intelligence');

    // Replace all content with the updated Markdown
    const replaceAll = utils.replaceAll(updatedMarkdown);
    replaceAll(ctx);
}

// Wait for the editor to finish creating
crepe.create().then(() => {
    console.log("Crepe editor is ready!");
    interactWithEditor(crepe.editor);
});
